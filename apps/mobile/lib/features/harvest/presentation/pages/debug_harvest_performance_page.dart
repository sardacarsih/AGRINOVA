...
class DebugHarvestPerformancePage extends StatefulWidget {
  ...
  @override
  State<DebugHarvestPerformancePage> createState() => _DebugHarvestPerformancePageState();
}

class _DebugHarvestPerformancePageState extends State<DebugHarvestPerformancePage> {
  final StopWatchTimer _stopWatch = StopWatchTimer();
  final ScrollController _scrollController = ScrollController();
  final List<PerformanceMetric> _metrics = [];

  // Form field controllers
  final TextEditingController _mandorController = TextEditingController(text: 'mandor1');
  final TextEditingController _blockController = TextEditingController(text: 'A1');
  final TextEditingController _employeeController = TextEditingController(text: 'EMP001');
  final TextEditingController _tbsController = TextEditingController(text: '500');
  ...
  void initState() {
    ...
    _generateTestData();
    _addMetric('Page Load', DateTime.now().difference(_formStartTime!).inMilliseconds);
  }
  ...
  Future<void> _runPerformanceTest() async {
    ...
    try {
      ...
      final totalTime = _completeTime!.difference(_formStartTime!).inMilliseconds;
      _addMetric('TOTAL TEST TIME', totalTime, details: 'Complete performance test finished');
  ...
  Future<void> _testFormValidation() async {
    ...
    await Future.delayed(Duration(milliseconds: _simulateSlowNetwork ? 200 : 50));
    ...
    await Future.delayed(Duration(milliseconds: _simulateSlowNetwork ? 100 : 20));

    final validationTime = DateTime.now().difference(validationStart).inMilliseconds;
    final totalTime = DateTime.now().difference(_validationStartTime!).inMilliseconds;

    _addMetric('Form Validation', totalTime, details:
  ...
  Future<void> _testLocalSave() async {
    ...
    await PerformanceService.instance.trackDatabaseOperation(
      ...
      () async {
        ...
        await Future.delayed(Duration(milliseconds: _simulateSlowNetwork ? 300 : 100));

        // Actual database operation (simulated)
        return await DatabaseService.instance.insertHarvestRecord(harvestData);
      },
      metadata: {
        'mandorId': _mandorController.text,
        'blockId': _blockController.text,
    ...
    _addMetric('Local Save', saveTime, details: 'Harvest record saved to SQLite');
  }
  ...
  Future<void> _testMultiEmployeeGeneration() async {
    ...
    await Future.delayed(Duration(milliseconds: _simulateSlowNetwork ? 50 : 10));

    final genTime = DateTime.now().difference(genStart).inMilliseconds;
    _addMetric('Multi-Employee Generation', genTime,
  ...
  Future<void> _testPhotoSimulation() async {
    ...
    await Future.delayed(Duration(milliseconds: _simulateSlowNetwork ? 1000 : 300));
    ...
    _addMetric('Photo Processing', photoTime, details: '$photoCount photos processed');
  }
  ...
  Future<void> _testSyncOperation() async {
    ...
    try {
      ...
      await Future.delayed(Duration(milliseconds: _simulateSlowNetwork ? 2000 : 500));

      // Simulate GraphQL mutation
      await Future.delayed(Duration(milliseconds: _simulateSlowNetwork ? 300 : 100));

      final syncTime = DateTime.now().difference(_syncStartTime!).inMilliseconds;
      _addMetric('Sync Operation', syncTime, details: 'Successfully synced to server');
  ...
  Future<void> _testBatchOperations() async {
    ...
    for (int i = 0; i < batchSize; i++) {
      await Future.delayed(Duration(milliseconds: _simulateSlowNetwork ? 20 : 5));
    }

    final batchTime = DateTime.now().difference(batchStart).inMilliseconds;
    _addMetric('Batch Operations', batchTime, details: '$batchSize records processed');
  }
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            Text('Average: ${(totalTime / _metrics.length).round()}ms per operation'),
            const SizedBox(height: 16),
            const Text('Status:', style: TextStyle(fontWeight: FontWeight.bold)),
            Text(
              totalTime < 3000 ? '🟢 Excellent' :
              totalTime < 5000 ? '🟡 Good' :
              totalTime < 8000 ? '🟠 Fair' : '🔴 Needs Improvement',
  ...
  void _exportMetrics() {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('Performance report exported to debug console')),
    );
  }

  void _runStressTest() async {
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('Starting stress test...')),
    );
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('Stress test completed!')),
    );
  }

  Future<void> _runBenchmarkTests() async {
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('Running benchmark tests...')),
    );
    ...
    try {
      ...
      _addMetric('Database Benchmark', dbResult.totalTimeMs,
          details: 'Success: ${dbResult.success}');

      // Form Input Benchmark
      final formResult = await PerformanceService.instance.runFormInputBenchmark();
      _addMetric('Form Input Benchmark', formResult.totalTimeMs,
          details: 'Success: ${formResult.success}');

      final totalBenchmarkTime = DateTime.now().difference(benchmarkStart).inMilliseconds;
      _addMetric('Total Benchmark Time', totalBenchmarkTime,
        ...
        content: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              const Text('Database Performance:', style: TextStyle(fontWeight: FontWeight.bold)),
              if (dbResult.success) ...[
                for (final entry in dbResult.results.entries)
                  Text('  ${entry.key}: ${entry.value}ms'),
              ] else
                Text('  Error: ${dbResult.error}', style: TextStyle(color: Colors.red)),

              const SizedBox(height: 16),

              const Text('Form Input Performance:', style: TextStyle(fontWeight: FontWeight.bold)),
              if (formResult.success) ...[
                for (final entry in formResult.results.entries)
                  Text('  ${entry.key}: ${entry.value}ms'),
              ] else
                Text('  Error: ${formResult.error}', style: TextStyle(color: Colors.red)),
            ],
          ),
        ),
        actions: [
        ...
        children: [
          ...
          Container(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                const Text('Network Simulation', style: TextStyle(fontWeight: FontWeight.bold)),
                const SizedBox(height: 8),
                Wrap(
                  spacing: 8,
                  children: [
                    FilterChip(
                      ...
                      onSelected: (value) => setState(() => _simulateSlowNetwork = value),
                    ),
                    FilterChip(
                      label: const Text('Offline Mode'),
                      selected: _simulateOfflineMode,
                      onSelected: (value) => setState(() => _simulateOfflineMode = value),
                    ),
                    FilterChip(
                      label: const Text('Sync Failure'),
                      selected: _simulateSyncFailure,
                      onSelected: (value) => setState(() => _simulateSyncFailure = value),
                    ),
                  ],
                ),
                const SizedBox(height: 8),
                  ...
                  children: [
                    ...
                    SizedBox(
                      ...
                      child: TextField(
                        ...
                        keyboardType: TextInputType.number,
                        onChanged: (value) => setState(() => _employeeCount = int.tryParse(value) ?? 1),
                        controller: TextEditingController(text: _employeeCount.toString()),
                      ),
                    ),
                  ],
                ),
  ...
  Widget _buildStatusChip() {
    if (_metrics.isEmpty) {
      return const Chip(label: Text('Ready', style: TextStyle(color: Colors.white)), backgroundColor: Colors.grey);
    }
    ...
    if (lastMetric.operation.contains('ERROR') || lastMetric.operation.contains('FAILED')) {
      color = Colors.red;
      status = 'Error';
...
class PerformanceMetricCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    Color getColor() {
      if (metric.operation.contains('ERROR') || metric.operation.contains('FAILED')) {
