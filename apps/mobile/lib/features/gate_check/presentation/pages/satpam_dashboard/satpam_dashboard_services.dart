...
import '../../../../../core/services/role_based_sync_service.dart';
...
class SatpamDashboardServices {
  ...
  static Future<SatpamDashboardServiceState> initializeServices() async {
    try {
      _logger.i('Initializing SatpamDashboard services using Dependency Injection');
  ...
  static Future<SatpamDashboardServiceState> _initializeFallbackServices() async {
    try {
      ...
      await connectivityService.initialize(); // Initialize for connectivity detection
      final jwtStorageService = JWTStorageService();
      
      _logger.i('Fallback services initialized - dashboard will operate in offline-only mode');
      return SatpamDashboardServiceState(
        databaseService: databaseService,
        connectivityService: connectivityService,
        jwtStorageService: jwtStorageService,
  ...
  static Future<SatpamDashboardData> loadDashboardData({
    ...
    try {
      ...
      if (serviceState.databaseService != null) {
        ...
        recentAccessLogs = await _loadRecentAccessLogs(serviceState.databaseService!);
  ...
  static Future<Map<String, dynamic>> _getPhotoStats(EnhancedDatabaseService db) async {
    try {
      // Check if table exists first
      final tableCheck = await db.rawQuery("SELECT name FROM sqlite_master WHERE type='table' AND name='gate_check_photos'");
      ...
      final totalResult = await db.rawQuery('SELECT COUNT(*) as count FROM gate_check_photos');
      ...
      final pending = pendingResult.isNotEmpty ? pendingResult.first['count'] as int? ?? 0 : 0;
  ...
  static Future<int?> _getGuestLogCount(EnhancedDatabaseService db) async {
    try {
      final result = await db.rawQuery('SELECT COUNT(*) as count FROM gate_guest_logs');
  ...
  static Future<int?> _getEmployeeLogCount(EnhancedDatabaseService db) async {
    try {
      ...
      final result = await db.rawQuery('SELECT COUNT(*) as count FROM gate_employee_logs');
      return result.isNotEmpty ? result.first['count'] as int? : 0;
    } catch (e) {
  ...
  static Future<GateCheckStats?> _loadTodayStats(EnhancedDatabaseService db) async {
    try {
      ...
      final complianceRate = todayEntries > 0 
          ? ((todayEntries - (vehiclesInside * 0.1)) / todayEntries * 100).clamp(0.0, 100.0)
          : 100.0;
      ...
      return GateCheckStats(
        ...
        totalWeightOut: 0.0, // Would need weight data from actual implementation
        violationCount: accessLogsCount > 10 ? (accessLogsCount * 0.05).round() : 0,
        createdAt: now,
        updatedAt: now,
      );
    } catch (e) {
  ...
  static Future<List<GuestLog>> _loadRecentGuestLogs(EnhancedDatabaseService db) async {
    try {
  ...
  static Future<List<AccessLog>> _loadRecentAccessLogs(EnhancedDatabaseService db) async {
  ...
  static Future<List<Map<String, dynamic>>> loadHistoryData({
    ...
    try {
      ...
      for (final row in guestLogsResult) {
        ...
        _logger.w('HISTORY DEBUG: ID=${row['id']} INTENT=$generationIntent ENTRY=${row['entry_time']} EXIT=${row['exit_time']}');
        ...
        void addGuestItem(String type, int timestamp) {
          ...
          historyItems.add({
            ...
            'time': '${displayTime.hour.toString().padLeft(2, '0')}:${displayTime.minute.toString().padLeft(2, '0')}',
            'date': '${displayTime.year}-${displayTime.month.toString().padLeft(2, '0')}-${displayTime.day.toString().padLeft(2, '0')}',
            'timestamp': timestamp,
        ...
        if (entryTime != null && (actionFilter == 'all' || actionFilter == 'entry')) {
        ...
        if (exitTime != null && (actionFilter == 'all' || actionFilter == 'exit')) {
        ...
        if (entryTime == null && exitTime == null) {
           final createdAt = row['created_at'] as int? ?? DateTime.now().millisecondsSinceEpoch;
           if (actionFilter == 'all' || actionFilter == generationIntent.toLowerCase()) {
      ...
      try {
        final tableCheck = await db.rawQuery("SELECT name FROM sqlite_master WHERE type='table' AND name='gate_employee_logs'");
        ...
        _logger.w('Employee logs table error (might not exist yet): ${e.toString()}');
      }
      ...
      _logger.d('Loaded ${historyItems.length} history items (Guests + Employees)');
      return historyItems;
  ...
  static Future<void> performManualSync({
    ...
    try {
      ...
      if (serviceState.syncService != null) {
        try {
          final result = await serviceState.syncService!.syncNow(forceFullSync: true);
  ...
  static Future<void> handleGuestRegistration({
    ...
    try {
      ...
      if (serviceState.databaseService != null) {
        ...
        final gateName = formData.posNumber.isNotEmpty ? formData.posNumber : 'MAIN_GATE';

        final guestLogData = {
          'driver_name': formData.guestName!,
          'destination': formData.purposeOfVisit ?? 'Visit',
          'vehicle_plate': formData.vehiclePlate,
          'vehicle_type': formData.vehicleType.isNotEmpty ? formData.vehicleType : 'Lainnya',
          'gate_position': gateName,
          'created_by': await _getUserIdFromJWT() ?? 'system_user',
          'company_id': await _getCompanyIdFromJWT() ?? '',
          'generation_intent': generationIntent,
          'entry_time': generationIntent == 'ENTRY' ? now.millisecondsSinceEpoch : null,
          'exit_time': generationIntent == 'EXIT' ? now.millisecondsSinceEpoch : null,
          ...
          'cargo_volume': formData.loadVolume.isNotEmpty ? formData.loadVolume : null,
          'cargo_owner': formData.loadOwner.isNotEmpty ? formData.loadOwner : null,
          'estimated_weight': formData.estimatedWeight,
          'delivery_order_number': formData.doNumber?.isNotEmpty == true ? formData.doNumber : null,
          'notes': formData.notes?.isNotEmpty == true ? formData.notes : null,
          'registration_source': 'MANUAL',
        };
        
        final newGuestId =
            await serviceState.databaseService!.createGuestLog(guestLogData);
  ...
  static Future<String?> handlePhotoCapture({
    ...
    try {
      ...
      if (vehiclePlate?.isEmpty ?? true) {
        throw Exception('Mohon isi nomor plat kendaraan terlebih dahulu sebelum mengambil foto');
      }

      return await _showCameraInterface(context, vehiclePlate!, serviceState, note);
      ...
      final photo = await Navigator.of(context).push<GateCheckPhoto>(
        MaterialPageRoute(
          builder: (context) => VehicleCameraWidget(
            ...
            notes: note.isNotEmpty ? note : 'Entry photo for guest registration',
            onPhotoTaken: (GateCheckPhoto capturedPhoto) {
              // Photo captured successfully - VehicleCameraWidget handles navigation
              _logger.d('Photo captured in callback: ${capturedPhoto.filePath}');
            },
            onCancel: () {
  ...
  static Future<Map<String, dynamic>?> handleQRGeneration({
    ...
    String? existingGuestLogId, // ID untuk regenerate (cetak ulang tanpa duplikasi)
  }) async {
    try {
      final isRegeneration = existingGuestLogId != null && existingGuestLogId.isNotEmpty;
      _logger.i('Generating QR code for guest: ${formData.guestName} with intent: $generationIntent (regenerate: $isRegeneration)');
      ...
      if (!isRegeneration && generationIntent == 'ENTRY' && serviceState.databaseService != null) {
        // cleaned plate
        ...
        final activeLogs = await serviceState.databaseService!.query(
          'gate_guest_logs',
          where: 'vehicle_plate = ? AND generation_intent = ? AND exit_time IS NULL',
        ...
        if (activeLogs.isNotEmpty) {
          ...
          _logger.i('Duplicate vehicle entry attempt: $cleanPlate - reusing existing session');
      ...
      if (guestId == null && serviceState.databaseService != null) {
        ...
        guestId = await serviceState.databaseService!.createGuestLog(guestLogData);
        _logger.i('Guest data saved to database with ID: $guestId for intent: $generationIntent');
        ...
        _logger.i('Regenerating QR for existing guest log ID: $guestId (no new database record created)');
      }
        ...
        if (guestId != null && serviceState.databaseService != null) {
          try {
            await serviceState.databaseService!.update(
              'gate_guest_logs',
              {'qr_code_data': qrData, 'updated_at': DateTime.now().millisecondsSinceEpoch},
              where: 'guest_id = ?',
              whereArgs: [guestId],
            );
            _logger.i('QR data persisted to guest log: $guestId');
          } catch (e) {
            _logger.w('Failed to persist QR data to guest log (non-fatal)', error: e);
          }
        }

        _logger.i('QR code generated successfully for guest: ${formData.guestName} (regenerate: $isRegeneration)');
        return {
          'qrData': qrData,
  ...
  static Future<bool> handleEmployeeQR({
    ...
    try {
      ...
      final accessLogData = {
        'log_id': logId,
        'employee_id': employeeData['nik'] ?? 'UNKNOWN', // Fallback as employee_id is required
      ...
      await serviceState.databaseService!.insertWithId('gate_employee_logs', accessLogData);
      
      _logger.i('Employee access log created: $logId - ${employeeData['name']}');
  ...
  static bool isOfflineMode(SatpamDashboardServiceState serviceState) {
    return serviceState.isOfflineMode || serviceState.connectivityService == null;
  }
  ...
  static Future<int> _storePhotosForGuestLog({
    ...
    for (int i = 0; i < photoPaths.length; i++) {
      ...
      try {
        ...
        _logger.i('Photo stored to gate_check_photos: $fileName ($photoType) for guest: $guestLogId');
        savedCount++;
      } catch (e) {
        _logger.e('Failed to store photo to gate_check_photos: $filePath', error: e);
      }
    }
    return savedCount;
  }
  ...
  static Future<String> _generateGuestQRToken(
    ...
    try {
      ...
      if (currentUserId == null) {
        _logger.w('No authenticated user found, using null for QR token creation');
      ...
      final token = await qrService.generateGuestToken(
        ...
        cargoVolume: formData.loadVolume.isNotEmpty ? formData.loadVolume : null, // "Seperempat", "Setengah", "Penuh"
        vehicleType: formData.vehicleType.isNotEmpty ? formData.vehicleType : null,
      ...
      _logger.d('Generated QR token for guest: ${formData.guestName ?? formData.driverName}');
      return qrData;
    } catch (e) {
