...
class RoleService {
  ...
  static bool canPerformGateCheck(String role) {
    return hasPermission(role, 'gate_check') ||
        hasPermission(role, 'gate_check_input');
  }
  ...
  static List<String> getDashboardFeatures(String role, {String? mandorType}) {
    ...
    final permissions = getRolePermissions(role);
    final isMaintenanceMandor = isMandorPerawatan(role, mandorType: mandorType);
  ...
  static Map<String, dynamic> getOfflineCapabilities(String role) {
    ...
    return {
      ...
      'canInputData':
          permissions.contains('harvest_input') ||
          permissions.contains('gate_check'),
      'canApproveOffline': permissions.contains('harvest_approval'),
      'canViewReports': permissions.any(
        (p) => p.contains('monitoring') || p.contains('reporting'),
      ),
